...
window.onload = function(){
    ...
    var userAcceptanceCheckBoxObjectParentNode = userAcceptanceCheckBoxObject.parentNode;
...
function checkTitleValue(titleObject){
    if(checkIfEmpty(titleObject)){
        setInvalid(titleObject, `${titleObject.id} box field must not be empty!!!`);
...
function checkIfEmpty(field){
    if(isEmpty(field.value.trim())){return true;}
    else
        return false;
}
...
function checkInputTextFields(objectArray){
    for(var i = 0; i < objectArray.length; i++){
        if(checkIfEmpty(objectArray[i])){
         ...
         if (objectArray[i].id == "Checkbox") {
            if(checkAcceptanceCheckBox(objectArray[i])){
                setInvalid(objectArray[i],`${objectArray[i].id} must be checked!!!` );
                event.preventDefault();
            }
        }
